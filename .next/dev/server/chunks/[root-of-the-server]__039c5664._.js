...
async function GET() {
    try {
        ...
        const { data: lists, error } = await supabase.from("lists").select("*").order("created_at", {
            ascending: false
        });
        if (error) {
        ...
        const listsWithCounts = await Promise.all((lists || []).map(async (list)=>{
            const { count, error: countError } = await supabase.from("list_memberships").select("*", {
                count: "exact",
                head: true
            }).eq("list_id", list.id);
            if (countError) {
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(listsWithCounts);
    } catch (error) {
        console.error("[v0] Error fetching lists:", error);
...
async function POST(request) {
    try {
        ...
        if (listType === "SMART" && filtersJson) {
            insertData.filters_json = typeof filtersJson === "string" ? filtersJson : JSON.stringify(filtersJson);
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__039c5664._.js.map